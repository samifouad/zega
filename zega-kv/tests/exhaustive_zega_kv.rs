...
#[test]
fn del_on_expired_but_not_yet_read_key_returns_false() {
    // Expired keys are logically absent even before another read removes them.
    let kv = KvStore::new();
    let mut entries = HashMap::new();
    entries.insert(
        "stale".to_string(),
...
// ===========================================================================
// TTL — boundary semantics: expiry is inclusive (`expires_at <= now`)
// ===========================================================================
...
#[test]
fn get_entry_expiring_exactly_now_is_absent() {
    // Expiry is inclusive, so an entry expiring now is already absent.
    let kv = KvStore::new();
    let now = now_secs();
    let mut entries = HashMap::new();
    entries.insert(
...
#[test]
fn set_with_zero_ttl_reads_as_absent() {
    // ttl_secs Some(0) expires immediately and is filtered from reads/snapshots.
    let kv = KvStore::new();
    kv.set("z".to_string(), Value::Int(9), Some(0));
    assert_eq!(kv.get("z"), None);
    assert!(!kv.snapshot().contains_key("z"));
}

// ===========================================================================
// TTL — mutating ops treat expired entries as absent
// ===========================================================================
...
#[test]
fn incr_on_expired_int_creates_fresh_counter() {
    let kv = KvStore::new();
    let mut entries = HashMap::new();
    entries.insert(
        "c".to_string(),
    ...
    kv.restore(entries);
    assert_eq!(kv.incr("c"), Some(Value::Int(1)));
    assert_eq!(kv.get("c"), Some(Value::Int(1)));
}
...
#[test]
fn lpush_onto_expired_list_creates_fresh_list() {
    let kv = KvStore::new();
    let mut entries = HashMap::new();
    entries.insert(
        "l".to_string(),
...
#[test]
fn lrange_on_expired_list_is_absent() {
    let kv = KvStore::new();
    let mut entries = HashMap::new();
    entries.insert(
        "l".to_string(),
...
#[test]
fn ltrim_on_expired_list_returns_false() {
    let kv = KvStore::new();
    let mut entries = HashMap::new();
    entries.insert(
        "l".to_string(),
...
// ===========================================================================
// SNAPSHOT / RESTORE — TTL fidelity and expired-entry filtering
// ===========================================================================
...
#[test]
fn snapshot_excludes_expired_but_unread_entries() {
    let kv = KvStore::new();
    let mut entries = HashMap::new();
    entries.insert(
        "stale".to_string(),
...
#[test]
fn snapshot_after_restore_filters_past_and_preserves_future_ttl() {
    let kv = KvStore::new();
    let future = now_secs() + 10_000;
    let mut entries = HashMap::new();
    entries.insert(
