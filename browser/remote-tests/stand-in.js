// A local stand-in for the zega-explorer2-api Fly Machine: every path is
// forwarded to a real `zega start --token-file`, which checks the Bearer token
// exactly as the Machine's server does. CORS answers only local origins, like
// the Machine's `--cors-origin http://localhost`. Every request is recorded so
// tests can assert what the browser sent; `delayNextMs` stands in for a wake.
...
export async function startRemote(token) {
  ...
  const server = createServer(async (request, response) => {
    ...
    const upstream = await fetch(`${zega.url}${url.pathname}${url.search}`, {
