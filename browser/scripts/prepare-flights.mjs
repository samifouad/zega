...
const source = `// Flights — airports and routes from OpenFlights, ODbL 1.0; the three CSV files are a derived database under the same licence.
// https://openflights.org/data.php
// Generated by browser/scripts/prepare-flights.mjs: the busiest airport of the ${COUNTRIES} busiest countries, the ${HUBS} busiest airports overall and ${HOME}, with every non-stop route among them stored once, from the smaller airport to the larger hub.
// Source SHA-256: ${Object.entries(hashes).map(([name, sha]) => `${name} ${sha}`).join(', ')}
schema {
  type Country {
    name: String
    iso: String<iso2>
