...
if (mode !== 'artifact') {
  for (const target of [[], ['--target', 'wasm32-unknown-unknown']]) {
    ...
    assert(!/^(worker(?:-sys|-macros)?|workerd|cloudflare(?:[-_][\w-]+)?) v/m.test(tree), `Cloudflare dependency leaked into zega-wasm:\n${tree}`);
  ...
  assert(!engine.dependencies.some(d => /^(worker(?:-sys|-macros)?|wasm-bindgen-futures|cloudflare(?:[-_][\w-]+)?)$/.test(d.name)), 'zega must not depend on Cloudflare or wasm-bindgen-futures');
