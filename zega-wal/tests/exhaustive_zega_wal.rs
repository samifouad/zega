...
fn flush_then_drop_in_group_mode_loses_nothing() {
    ...
    let path = dir.path().join("wal.bin");
    {
        let wal = Wal::with_group_commit(&path, false, Duration::from_secs(10), 1000).unwrap();
        // Large interval + large batch: rely on append's blocking durability
        // contract and on the drop-time worker flush.
        for i in 0..20 {
...
#[test]
fn legacy_length_prefix_overflowing_past_eof_stops_migration() {
    // Legacy entry whose declared length runs past EOF is treated as a torn
    // tail: migration keeps the complete prefix and drops the overflow.
    let dir = tempdir().unwrap();
    let path = dir.path().join("wal.bin");
    {
        let mut file = File::create(&path).unwrap();
...
fn appends_crossing_batch_boundary_are_all_durable() {
    let dir = tempdir().unwrap();
    let path = dir.path().join("wal.bin");
    // batch_size 4, huge interval: the batch-size notify_one path is the only
    // thing that can wake the worker. Write a non-multiple (10) to ensure the
    // trailing partial batch is also flushed before append returns.
    let wal = Wal::with_group_commit(&path, false, Duration::from_secs(30), 4).unwrap();
    for i in 0..10 {
        wal.append(&kv_set(&format!("c{i}"))).unwrap();
    }
    let recovered = kv_keys(&Wal::new(&path, false).unwrap().iter().unwrap());
    assert_eq!(recovered.len(), 10);
    for (i, k) in recovered.iter().enumerate() {
