...
fn parse_one(input: &str) -> Statement {
    ...
    assert_eq!(stmts.len(), 1, "expected exactly one statement for: {input}");
    stmts.remove(0)
}
...
fn lex_lte_vs_left_arrow_disambiguation() {
    ...
    assert_eq!(lex_all("<= <- <"), vec![Token::Lte, Token::LeftArrow, Token::Lt]);
}
...
fn lex_unicode_identifier_is_alphanumeric() {
    ...
    assert_eq!(
        lex_all("café"),
        vec![Token::Identifier("café".to_string())]
    );
...
fn lex_integer_dot_identifier_property_access_shape() {
    ...
    assert_eq!(
        lex_all("5.x"),
        vec![Token::Integer(5), Token::Dot, Token::Identifier("x".to_string())]
    );
}
...
fn lex_i64_max_parses() {
    assert_eq!(lex_all("9223372036854775807"), vec![Token::Integer(i64::MAX)]);
}
...
fn lex_string_unknown_escape_keeps_char() {
    ...
    assert_eq!(lex_all("'\\x'"), vec![Token::StringLiteral("x".to_string())]);
}
...
fn parse_return_property_access() {
    ...
    assert_eq!(
        ret.items[0].expr,
        Expr::PropertyAccess(Box::new(Expr::Identifier("n".to_string())), "name".to_string())
    );
}
...
fn parse_relationship_with_properties() {
    ...
    let (pattern, _, _) = as_match(&stmt);
    let rel = pattern[1].relationship.as_ref().unwrap();
    assert_eq!(rel.properties.get("stars"), Some(&Expr::Literal(Value::Int(5))));
}
...
fn parse_var_length_star_only() {
    ...
    let (pattern, _, _) = as_match(&stmt);
    let len = pattern[1].relationship.as_ref().unwrap().length.clone().unwrap();
...
fn parse_var_length_fixed() {
    ...
    let (pattern, _, _) = as_match(&stmt);
    let len = pattern[1].relationship.as_ref().unwrap().length.clone().unwrap();
    assert_eq!(len, RelationshipLength { min: 3, max: Some(3) });
}
...
fn parse_var_length_range() {
    ...
    let (pattern, _, _) = as_match(&stmt);
    let len = pattern[1].relationship.as_ref().unwrap().length.clone().unwrap();
    assert_eq!(len, RelationshipLength { min: 2, max: Some(5) });
}
...
fn parse_var_length_open_lower() {
    ...
    let (pattern, _, _) = as_match(&stmt);
    let len = pattern[1].relationship.as_ref().unwrap().length.clone().unwrap();
    assert_eq!(len, RelationshipLength { min: 1, max: Some(3) });
}
...
fn parse_var_length_no_kind() {
    ...
    let rel = pattern[1].relationship.as_ref().unwrap();
    assert!(rel.kinds.is_empty());
    assert_eq!(rel.length, Some(RelationshipLength { min: 1, max: Some(3) }));
    assert_eq!(pattern[1].direction, Some(Direction::Both));
}
...
fn parse_where_or_is_left_associative() {
    ...
    match where_clause.as_ref().unwrap() {
        Expr::BinaryOp(left, BinaryOperator::Or, _) => {
            assert!(matches!(left.as_ref(), Expr::BinaryOp(_, BinaryOperator::Or, _)));
        }
        other => panic!("expected top-level OR, got {other:?}"),
    }
}
...
fn parse_where_comparison_binds_tighter_than_and() {
    ...
    match where_clause.as_ref().unwrap() {
        Expr::BinaryOp(left, BinaryOperator::And, right) => {
            assert!(matches!(left.as_ref(), Expr::BinaryOp(_, BinaryOperator::Eq, _)));
            assert!(matches!(right.as_ref(), Expr::BinaryOp(_, BinaryOperator::Eq, _)));
        }
...
fn parse_limit_parameter() {
    ...
    match &stmts[0] {
        Statement::Match { limit, .. } => {
            assert_eq!(limit.as_ref().unwrap(), &Expr::Parameter("count".to_string()));
        }
        other => panic!("got {other:?}"),
    }
}
...
fn parse_order_by_then_limit_together() {
    ...
    let stmts = p.parse().unwrap();
    match &stmts[0] {
        Statement::Match { order_by, limit, .. } => {
...
fn parse_limit_absent_is_none() {
    ...
    let stmts = p.parse().unwrap();
    match &stmts[0] {
        Statement::Match { limit, order_by, .. } => {
...
fn parse_aggregate_function_name_case_insensitive() {
    ...
    assert!(matches!(
        ret.items[0].expr,
        Expr::Aggregate { function: AggregateFunction::Count, .. }
    ));
}
...
fn parse_multi_match_then_create() {
    let stmt = parse_one(
        "MATCH (a:Cat {id: $p}) MATCH (b:Cat {id: $c}) CREATE (a)-[:SUB]->(b)",
    );
    match &stmt {
        Statement::MatchCreate { match_pattern, create_pattern, .. } => {
...
fn parse_merge_with_on_create_set() {
    ...
    match &stmt {
        Statement::Merge { on_create, .. } => {
            ...
            assert_eq!(
                *target,
                Expr::PropertyAccess(Box::new(Expr::Identifier("n".to_string())), "created".to_string())
            );
...
fn parse_delete_multiple_identifiers() {
    ...
    match &stmt {
        Statement::Delete { identifiers } => {
            assert_eq!(identifiers, &vec!["a".to_string(), "b".to_string(), "c".to_string()]);
        }
        other => panic!("got {other:?}"),
    }
}
...
fn parse_kv_get_string_key() {
    ...
    match &stmt {
        Statement::KvGet { key } => {
            assert_eq!(key, &Expr::Literal(Value::String("session:123".to_string())));
        }
        other => panic!("got {other:?}"),
    }
}
...
fn parse_return_string_literal() {
    ...
    let (_, _, ret) = as_match(&stmt);
    assert_eq!(ret.items[0].expr, Expr::Literal(Value::String("hi".to_string())));
}
...
fn value_partial_ord_cross_type_is_none() {
    assert_eq!(Value::Int(1).partial_cmp(&Value::String("a".to_string())), None);
...
fn ast_relationship_length_equality() {
    assert_eq!(
        RelationshipLength { min: 1, max: Some(3) },
        RelationshipLength { min: 1, max: Some(3) }
    );
    assert_ne!(
        RelationshipLength { min: 1, max: None },
        RelationshipLength { min: 1, max: Some(1) }
    );
}
...
fn lex_leading_underscore_is_identifier_not_number() {
    ...
    assert_eq!(
        lex_all("_5"),
        vec![Token::Identifier("_5".to_string())]
    );
}
...
fn parse_var_length_equal_min_max_ok() {
    ...
    assert_eq!(
        ...
        Some(RelationshipLength { min: 3, max: Some(3) })
    );
}
...
fn parse_var_length_zero_bounds() {
    ...
    assert_eq!(
        pattern[1].relationship.as_ref().unwrap().length,
        Some(RelationshipLength { min: 0, max: Some(0) })
    );
}
...
fn parse_count_distinct_with_property_argument() {
    ...
    let (_, _, ret) = as_match(&stmt);
    match &ret.items[0].expr {
        Expr::Aggregate { function: AggregateFunction::Count, distinct, argument } => {
            assert!(*distinct);
            assert!(argument.is_some());
        }
        other => panic!("got {other:?}"),
...
fn parse_sum_distinct_is_allowed() {
    ...
    let (_, _, ret) = as_match(&stmt);
    match &ret.items[0].expr {
        Expr::Aggregate { function: AggregateFunction::Sum, distinct, argument } => {
            assert!(*distinct);
            assert!(argument.is_some());
        }
        other => panic!("got {other:?}"),
...
fn parse_collect_with_property_argument() {
    ...
    assert!(matches!(
        ret.items[0].expr,
        Expr::Aggregate { function: AggregateFunction::Collect, distinct: false, .. }
    ));
}
...
fn parse_where_bare_identifier_expression() {
    ...
    let (_, where_clause, _) = as_match(&stmt);
    assert_eq!(where_clause.as_ref().unwrap(), &Expr::Identifier("n".to_string()));
}
...
fn parse_where_bare_property_access_expression() {
    ...
    assert_eq!(
        where_clause.as_ref().unwrap(),
        &Expr::PropertyAccess(Box::new(Expr::Identifier("n".to_string())), "active".to_string())
    );
}
...
fn parse_where_literal_only() {
    ...
    let (_, where_clause, _) = as_match(&stmt);
    assert_eq!(where_clause.as_ref().unwrap(), &Expr::Literal(Value::Bool(true)));
}
...
fn parse_match_create_carries_where_across() {
    let stmt = parse_one(
        "MATCH (a:A) MATCH (b:B) WHERE a.id = $x CREATE (a)-[:R]->(b)",
    );
    match &stmt {
        Statement::MatchCreate { match_pattern, where_clause, create_pattern } => {
            assert_eq!(match_pattern.len(), 2);
            assert!(where_clause.is_some());
            assert_eq!(create_pattern.len(), 2);
        }
...
fn parse_error_message_variant_displays_inner_string() {
    ...
    let shown = format!("{err}");
    assert!(shown.contains("minimum exceeds maximum"), "Display: {shown}");
}
